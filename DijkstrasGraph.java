...
/**
 * Class that implements Dijkstra's algorithm using Paths/Vertexes/Edges
 * 
 * @author jacksonzhao
 *
 * @param <T> - Type of Data stored in the Graph
 */
public class DijkstrasGraph<T> implements GraphADT<T> {
  ...
  protected class Path implements Comparable<Path> {
    ...
    public Path(Path copyPath, Edge extendBy) {
      ...
      // Iterate through the copyPath's dataSequence to create a deep copy
  ...
  protected Path dijkstrasShortestPath(T start, T end) {
    // Checks if the Graph contains contains the Vertex with data start and Vertex with data end
    if (!containsVertex(start) || !containsVertex(end)) {
      throw new NoSuchElementException("The start or end is not contained in the graph.");
    }

    // Initalizes the necessary variables
    PriorityQueue<Path> pq = new PriorityQueue<Path>();
    Vertex startVertex = vertices.get(start);
    Path startPath = new Path(startVertex);

    // If the start & data are equal return startPath
    if (startPath.dataSequence.equals(end)) {
      return startPath;
    }

    // Add startPath to our Priority Queue
    pq.add(startPath);

    // Keeps track of the visited Vertexes, so we do not create an infinite loop
    LinkedList<List<T>> visitedData = new LinkedList<List<T>>();

    while (!pq.isEmpty()) {
      // Poll the minimum Path from the PriorityQueue
      Path min = pq.poll();

      // Mark that dataSequence visited
      visitedData.add(min.dataSequence);

      // Make sure we have not arrived at our end Vertex, if we have we found our shortestPath
      for (int i = 0; i < visitedData.size(); i++) {
        if (visitedData.get(i).contains(end)) {
          return min;
        }
      }

      // Goes through the edges out of the Path min and removes those targets already visited, this
      // ensures we do not fall into an infinite loop
      LinkedList<Edge> edgesLeaving = min.end.edgesLeaving;
      for (int k = edgesLeaving.size() - 1; k >= 0; k--) {
        for (int i = 0; i < visitedData.size(); i++) {
          if (visitedData.get(i).contains(edgesLeaving.get(k).target.data)) {
            edgesLeaving.remove(k);
            break;
          }
        }
      }

      // Creates Paths from the remaining edgesLeaving the Path min
      LinkedList<Path> paths = new LinkedList<>();
      for (int j = 0; j < edgesLeaving.size(); j++) {
        paths.add(new Path(min, edgesLeaving.get(j)));
      }

      // Adds the Paths to the PriorityQueue which sorts the Paths for us
    ...
    // Priority Queue is empty and we still have not found a Path therefore path doesn't exist
